...
2. **Assign write authority.** Specify which actors can write to each tier: agent, human editor, scheduled maintainer, workspace admin, CI job, or external sync. Default canonical knowledge to human-reviewed writes.
...
4. **Define contamination barriers.** Prevent agent conclusions, workspace-specific facts, private examples, and stale summaries from being written into the shared corpus.
...
7. **Add tests.** Create scenarios where the agent has a tempting memory note that conflicts with the corpus, a stale canonical page, a workspace-specific preference, and a missing source.
</process>
...
- Canonical knowledge cannot be contaminated by agent inference or workspace-private facts.
